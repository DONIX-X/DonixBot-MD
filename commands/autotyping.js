...
async function autotypingCommand(sock, chatId, message) {
    try {
        ...
        // Explicitly turn autotyping ON/OFF. With no argument, toggle the current state.
        if (args.length > 0) {
        ...
        // Save only the real setting. This prevents the old `disabled` flag from
        // becoming out of sync with `enabled` after reconnects or restarts.
        fs.writeFileSync(configPath, JSON.stringify({ enabled: config.enabled }, null, 2));
