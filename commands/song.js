...
async function getYtDlpDownloadByUrl(youtubeUrl) {
	const outputPath = path.join(__dirname, '../temp', `song-${Date.now()}-${Math.random().toString(36).slice(2)}.mp4`);
	...
	try {
		...
		return { buffer: audioBuffer, title: null };
	} finally {
		await fs.promises.rm(outputPath, { force: true }).catch(() => {});
	}
}

function isSupportedMediaBuffer(buffer) {
	if (!buffer || buffer.length < 4) return false;
	const signature = buffer.slice(0, 12);
	return signature.slice(0, 3).toString('ascii') === 'ID3' ||
		(buffer[0] === 0xFF && (buffer[1] & 0xE0) === 0xE0) ||
		signature.slice(0, 4).toString('ascii') === 'OggS' ||
		signature.slice(0, 4).toString('ascii') === 'RIFF' ||
		signature.slice(4, 8).toString('ascii') === 'ftyp';
}
...
async function songCommand(sock, chatId, message) {
    try {
		...
		for (const apiMethod of apiMethods) {
			try {
				...
				try {
					...
					// Validate buffer
					if (isSupportedMediaBuffer(audioBuffer)) {
						downloadSuccess = true;
						break; // Success! Exit the loop
					}
					throw new Error('Download response was not an audio or video file');
				} catch (downloadErr) {
					...
					try {
						...
						throw new Error('Stream response was not an audio or video file');
					} catch (streamErr) {
		...
		if (!downloadSuccess) {
			try {
				const localDownload = await getYtDlpDownloadByUrl(video.url);
				audioBuffer = localDownload.buffer;
				audioData = { title: localDownload.title };
				downloadSuccess = true;
			} catch (ytDlpErr) {
				console.log('yt-dlp fallback failed:', ytDlpErr.message);
			}
		}
		
		// If all APIs failed, throw error
		if (!downloadSuccess || !audioBuffer) {
			throw new Error('All song download sources failed.');
		}
		...
		} else if (err.message && err.message.includes('All song download sources failed')) {
			errorMessage = '❌ Song download failed. Please try again later.';
        }
