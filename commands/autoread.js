/**
 * Donix Bot - A WhatsApp Bot
 * read Command - matically read all messages
 */
...
const configPath = path.join(__dirname, '..', 'data', 'read.json');
    ...
    try {
        ...
        if (args.length > 0) {
            ...
            if (action === 'on' || action === 'enable') {
                ...
                await sock.sendMessage(chatId, {
                    text: '❌ Invalid option! Use: .read on/off',
                    contextInfo: {
                        forwardingScore: 1,
                        isForwarded: true,
                        }
        ...
        await sock.sendMessage(chatId, {
            text: `✅ -read has been ${config.enabled ? 'enabled' : 'disabled'}!`,
            contextInfo: {
                forwardingScore: 1,
                isForwarded: true,
                }
...
function isBotMentionedInMessage(message, botNumber) {
    ...
    if (textContent) {
        ...
        const botNames = [global.botname?.toLowerCase(), 'bot', 'donix', 'donix bot', 'donix bot md'];
